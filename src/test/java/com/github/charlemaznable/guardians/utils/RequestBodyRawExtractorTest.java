...
public class RequestBodyRawExtractorTest {
    ...
    public void testRequestBodyRawExtractor() {
        ...
        val rawExtractor1 = (RequestBodyRawExtractor) BODY_RAW.extractor(newArrayList(""));
        assertEquals("UTF-8", rawExtractor1.getCharsetName());
        assertEquals("HTTP BODY 内容", rawExtractor1.extract(request).get(RAW));

        request.setContent(bytes("HTTP BODY 内容"));

        val rawExtractor2 = (RequestBodyRawExtractor) BODY_RAW.extractor(newArrayList(""), null, ISO_8859_1.name());
        assertEquals("ISO-8859-1", rawExtractor2.getCharsetName());
        assertEquals("HTTP BODY 内容", string(bytes(rawExtractor2.extract(request).get(RAW).toString(), ISO_8859_1)));
    }
}
